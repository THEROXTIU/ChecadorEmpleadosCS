...
def login(request):
    ...
    #Si no hay una sesión iniciada
        ...
        if request.method == "POST":
            ...
            datosUsuario = Empleados.objects.filter(correo=correousuario, contraseña = pwd)
...
def verAreas(request):
    ...
    if "idSesion" in request.session:
        ...
        for area in infoAreas:
            ...
            empleadosEnArea = Empleados.objects.filter(id_area_id__id_area = areaInt, activo = "A")#filtro de los empleados que esten dentro de un area especifica
...
def guardarImagen(request):
    ...
    if request.method == "POST":
        ...
        email_destino = ['sistemas@customco.com.mx', 'egutierrez@customco.com.mx', 'edgar.gutierrez@customco.com.mx',correoEmpleado]
...
def agregarSoporteEquipoComputo(request):
    if "idSesion" in request.session:
        ...
        if request.method == "POST":
            ...
            operaciones = request.POST.getlist('operacion')  #ES un arreglo de todo lo que se manda en el select
